    ...
    def load_tasks(self):
        try:
            with open(self.FILENAME, "r") as file:
                data = json.load(file)
                self._tasks = [Task(item["id"], item["description"], item["completed"]) for item in data] #transformamos la clase json a task
                if self._tasks: # obtener la ultima tarea y sumarle 1 si hay tareas
